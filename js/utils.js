...
function canBuyBuyable(layer, id) {
	...
	return (
		b.unlocked && run(b.canAfford, b) && player[layer].buyables[id].lt(b.purchaseLimit) && !tmp[layer].deactivated
	);
}
...
function goBack(layer) {
	...
	if (player.navTab === "none" && (tmp[layer]?.row == "side" || tmp[layer].row == "otherside"))
...
function updateMilestones(layer) {
	...
	for (id in layers[layer].milestones) {
		if (!hasMilestone(layer, id) && layers[layer].milestones[id].done()) {
			...
			if (
				(tmp[layer].milestonePopups || tmp[layer].milestonePopups === undefined) &&
				!options.hideMilestonePopups
			)
				doPopup(
					"milestone",
