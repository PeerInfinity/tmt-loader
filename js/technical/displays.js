	...
	if (tmp[layer].type == "normal")
		return `${
			...
			tmp[layer].resetGain.lt(100) && player[layer].points.lt(1e3)
				? `<br><br>Next at ${
						tmp[layer].roundUpCost ? formatWhole(tmp[layer].nextAt) : format(tmp[layer].nextAt)
				  } ${tmp[layer].baseResource}`
				: ""
		}`;
	if (tmp[layer].type == "static")
		return `${
			tmp[layer].resetDescription !== undefined ? tmp[layer].resetDescription : "Reset for "
		}+<b>${formatWhole(tmp[layer].resetGain)}</b> ${tmp[layer].resource}<br><br>${
...
function updateOomps(diff) {
	...
	if (pp.gt(lp)) {
		if (pp.gte("10^^8")) {
			...
			while (pp.div(lp).max(1).log(10).div(diff).gte("100") && tmp.other.oompsMag <= 5 && lp.gt(0)) {
				pp = pp.max(1).log(10);
				lp = lp.max(1).log(10);
				tmp.other.oomps = pp.sub(lp).div(diff);
				tmp.other.oompsMag++;
...
function constructBarStyle(layer, id) {
	...
	let tempProgress;
	if (bar.progress instanceof Decimal) tempProgress = (1 - Math.min(Math.max(bar.progress.toNumber(), 0), 1)) * 100;
	else tempProgress = (1 - Math.min(Math.max(bar.progress, 0), 1)) * 100;
