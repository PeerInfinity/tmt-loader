		...
		if (
			...
			if (
				...
				if (layer == "fishing") {
					...
					const resolvedName =
						ach.fishName ||
						(typeof ach.name === "function" ? ach.name() : ach.name);

					const fish = FISHING_FISHES.find((f) => f.name === resolvedName);
					if (!fish) return;

					const color = RARITY_MAP[fish.rarity].color;
					doPopup("achievement", fish.name, "New Fish!", 4, color);
				} else if (layer == "sa") {
					doPopup(
						"achievement",
...
function doPopup(
	...
	switch (type) {
		case "achievement":
			...
			break;
		case "challenge":
			popupTitle = "Challenge Completed!";
