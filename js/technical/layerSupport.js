...
function setupLayer(layer){
    ...
    // NEW STUFF
    if (layers[layer].spells){
        setRowCol(layers[layer].spells)
        for (thing in layers[layer].spells){
            if (isPlainObject(layers[layer].spells[thing])){
                layers[layer].spells[thing].id = thing
                layers[layer].spells[thing].layer = layer
                if (layers[layer].spells[thing].unlocked == undefined)
                    layers[layer].spells[thing].unlocked = true
            }
        }
    }
...
function setSpellRowCol(spells) {
    if (spells.rows && spells.cols) return
    let maxRow = 0
    let maxCol = 0
    for (sp in spells) {
        if (!isNaN(sp)) {
            let f = sp%100
            if (Math.floor(f/10) > maxRow) maxRow = Math.floor(f/10)
            if (f%10 > maxCol) maxCol = f%10
        }
    }
    spells.rows = maxRow
    spells.cols = maxCol
