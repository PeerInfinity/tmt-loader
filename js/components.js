...
// data = a function returning the content (actually HTML)
	...
	Vue.component('upgrade', {
		props: ['layer', 'data'],
		template: `
		<button v-if="tmp[layer].upgrades && tmp[layer].upgrades[data]!== undefined && (tmp[layer].upgrades[data].unlocked || hasUpgrade(layer, tmp[layer].upgrades[data].id))" :id='"upgrade-" + layer + "-" + data' v-on:click="buyUpg(layer, data)" v-bind:class="{ [layer]: true, tooltipBox: true, upg: true, bought: hasUpgrade(layer, data), locked: (!(canAffordUpgrade(layer, data))&&!hasUpgrade(layer, data)), can: (canAffordUpgrade(layer, data)&&!hasUpgrade(layer, data))}"
			v-bind:style="[((!hasUpgrade(layer, data) && canAffordUpgrade(layer, data)) ? {'background-color': tmp[layer].color, 'animation': 'upgradeBuy 1s infinite'} : {}), tmp[layer].upgrades[data].style]">
	...
	Vue.component('milestone', {
		props: ['layer', 'data'],
		template: `
		<td v-if="tmp[layer].milestones && tmp[layer].milestones[data]!== undefined && milestoneShown(layer, data) && (tmp[layer].milestones[data].unlocked || hasMilestone(layer, tmp[layer].milestones[data].id))" v-bind:style="[tmp[layer].milestones[data].style]" v-bind:class="{milestone: !hasMilestone(layer, data), tooltipBox: true, milestoneDone: hasMilestone(layer, data)}">
	...
	Vue.component('prestige-button', {
		...
		<button v-if="(tmp[layer].type !== 'none')" v-bind:class="{ [layer]: true, reset: true, locked: !tmp[layer].canReset, can: tmp[layer].canReset}"
			v-bind:style="[tmp[layer].canReset ? {'background-color': tmp[layer].color, 'width': '270px', 'border-radius': '100px', 'corner-type': 'round'} : {'width': '270px', 'border-radius': '100px', 'corner-type': 'round'}, tmp[layer].componentStyles['prestige-button']]"
	...
	Vue.component('main-display', {
		props: ['layer', 'data'],
		template: `
		<div><span v-if="player[layer].points.lt('1e1000')">You have </span><span v-if="tmp[layer].prefix!=null"><h2 v-bind:style="{'color': tmp[layer].color, 'text-shadow': '0px 0px 10px ' + tmp[layer].color}">{{tmp[layer].prefix}}</h2></span><h2 v-bind:style="{'color': tmp[layer].color, 'text-shadow': '0px 0px 10px ' + tmp[layer].color}">{{data ? format(player[layer].points, data) : format(player[layer].points)}}</h2><span v-if="tmp[layer].currencyOff!=true"> {{tmp[layer].resource}}</span><span v-if="layers[layer].effectDescription">, <span v-html="run(layers[layer].effectDescription, layers[layer])"></span></span><br><br></div>
		`
	})
