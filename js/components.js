...
// data = a function returning the content (actually HTML)
			...
			<div v-for="row in (data === undefined ? tmp[layer].upgrades.rows : data)" class="upgRow">
				<div v-for="col in tmp[layer].upgrades.cols">
					<div v-if="tmp[layer].upgrades[row*10+col]!== undefined && tmp[layer].upgrades[row*10+col].unlocked" class="upgAlign">
						<upgrade :layer = "layer" :data = "row*10+col" v-bind:style="tmp[layer].componentStyles.upgrade">
						</upgrade>
					</div>
				</div>
			</div>
	...
	Vue.component('upgrade', {
		props: ['layer', 'data'],
		template: `
		<button v-if="tmp[layer].upgrades && tmp[layer].upgrades[data]!== undefined && tmp[layer].upgrades[data].unlocked" 
			:id='"upgrade-" + layer + "-" + data' v-on:click="buyUpg(layer, data)" 
			v-bind:class="{ [layer]: true, tooltipBox: true, upg: true, bought: hasUpgrade(layer, data), 
			locked: (!(canAffordUpgrade(layer, data))&&!hasUpgrade(layer, data)), 
			can: (canAffordUpgrade(layer, data)&&!hasUpgrade(layer, data))}"
			v-bind:style="[((!hasUpgrade(layer, data) && canAffordUpgrade(layer, data)) ? {'background-color': tmp[layer].color} : {}), 
			...
			<span v-else>
				<span v-if= "tmp[layer].upgrades[data].title">
					<h3 v-html="tmp[layer].upgrades[data].title">
					</h3>
					<br>
				</span>
			...
			<tooltip v-if="tmp[layer].upgrades[data].tooltip" :text="tmp[layer].upgrades[data].tooltip">
			</tooltip>
			</button>
		`
	})
	...
	Vue.component('milestone', {
		...
		<td v-if="tmp[layer].milestones && tmp[layer].milestones[data]!== undefined && milestoneShown(layer, data) && tmp[layer].milestones[data].unlocked" v-bind:style="[tmp[layer].milestones[data].style]" v-bind:class="{milestone: !hasMilestone(layer, data), tooltipBox: true, milestoneDone: hasMilestone(layer, data)}">
			...
			<span v-html="run(layers[layer].milestones[data].effectDescription, layers[layer].milestones[data])"></span>
			<span v-if="layers[layer].milestones[data].effectDisplay"><br>Currently: <span v-html="run(layers[layer].milestones[data].effectDisplay, layers[layer].milestones[data])"></span></span>
 ...
 Vue.component('spells', {
	props: ['layer', 'data'],
	template: `
	<div v-if="tmp[layer].spells" class="spellTable">
		<div v-for="row in (data === undefined ? tmp[layer].spells.rows : data)" class="spellRow">
			<div v-for=col in tmp[layer].spells.cols">
				<div v-if="tmp[layer].spells[row*10+col]!== undefined && tmp[layer].spells[row*10+col].unlocked" class="spellAlign">
					<spell :layer = "layer" :data = "row*10+col" v-bind:style="tmp[layer].componentStyles.spell">
					</spell>
				</div>
			</div>
		</div>
 ...
 Vue.component('spell', {
	props: ['layer', 'data'],
	template: `
	<button v-if="tmp[layer].upgrades && tmp[layer].spells[data]!== undefined && tmp[layer].spells[data].unlocked" 
		:id='"upgrade-" + layer + "-" + data' v-on:click="castSpell(layer, data)" 
		v-bind:class="{ [layer]: true, tooltipBox: true, spell: true, active: hasSpell(layer, data), 
		locked: (!(canCastSpell(layer, data))&&!hasSpell(layer, data)),
		can: (canCastSpell(layer, data)&&!hasSpell(layer, data))}" 
		v-bind:style="[((!hasSpell(layer, data) && canCastSpell(layer, data)) ? {'background-color': tmp[layer].color} : {}),
		tmp[layer].upgrades[data].style]">
			<span v-if="layers[layer].spells[data].fullDisplay"
				v-html="run(layers[layer].spells[data].fullDisplay, layers[layer].upgrades[data])">
			</span>
			<span v-else>
				<span v-if="tmp[layer].spells[data].title">
					<h3 v-html="tmp[layer].spells[data].title">
					</h3>
					<br>
				</span>
				<span v-html="tmp[layer].spells[data].description">
				</span>
				<span v-if="layers[layer].spells[data].effectDisplay">
					<br>Currently:
					<span v-html="run(layers[layer].spells[data].effectDisplay, layers[layer].spells[data])">
					</span>
				</span>
				<br><br>
				Cost: {{ formatWhole(tmp[layer].spells[data].cost) }}
				{{ tmp[layer].spells[data].currencyDisplayName ? tmp[layer].spells[data].currencyDisplayName : tmp[layer].resource)}}
			</span>
			<tooltip v-if="tmp[layer].spells[data].tooltip" :text="tmp[layer].spells[data].tooltip">
			</tooltip>
		</button>
	`
 })