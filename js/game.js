...
function layerDataReset(layer, keep = [], maxBuyable = 0) {
	...
	for (const key in player[layer].buyables) {
		if (player[layer].buyables.hasOwnProperty(key)) {
			if (player[layer].buyables[key].gt(maxBuyable)) {
				player[layer].buyables[key] = new Decimal(maxBuyable)
			}
		}
	}
...
function autoCastSpell(layer) {
	if (!tmp[layer].spells) return
