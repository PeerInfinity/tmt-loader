...
function getNextAt(layer, canMax = false, useType = null) {
	...
	if (type == "static") {
		...
		let amt = player[layer].points
			.plus(canMax && tmp[layer].baseAmount.gte(tmp[layer].nextAt) ? tmp[layer].resetGain : 0)
			.div(tmp[layer].directMult);
		let extraCost = Decimal.pow(tmp[layer].base, amt.pow(tmp[layer].exponent).div(tmp[layer].gainExp)).times(
			tmp[layer].gainMult
		);
...
VERSION.withoutName =
	"v" + VERSION.num + (VERSION.pre ? " Pre-Release " + VERSION.pre : VERSION.pre ? " Beta " + VERSION.beta : "");
