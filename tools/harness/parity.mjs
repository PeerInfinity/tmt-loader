...
export async function parity(id, { ticks, diff, leg = 'idle', base, browser, mutant = false, profile = null, exclude = [], autoOpt = null }) {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'tmt-loader-parity-'));
  const stateFile = path.join(tmp, 'node.json');
  const node = runNode(id, { ticks, diff, leg, 'state-out': stateFile, profile, exclude: exclude.length ? exclude.join(',') : undefined, 'auto-opt': autoOpt || undefined });
  if (!node.ok) return { id, ok: false, error: `node: ${node.failed_at} ${node.error}` };
  const nodeJson = fs.readFileSync(stateFile, 'utf8');
  const page = await runPage(browser, base, id, { ticks, diff, leg, mutant, profile, exclude, autoOpt });
  ...
  return {
    ...
    ticks: node.ticks, gameSeconds: node.gameSeconds, diff, leg, profile: profile || 'off',
...
async function main() {
  const a = parseArgs(process.argv.slice(2), ['mutant']);
  const id = a._[0];
  ...
  let r;
  try { r = await parity(id, { ticks: Number(a.ticks ?? 1000), diff: Number(a.diff ?? 0.05), leg: a.leg || 'idle', mutant: !!a.mutant, base: a.base || server.url, browser, profile: a.profile || null, exclude: a.exclude ? a.exclude.split(',') : [], autoOpt: a['auto-opt'] || null }); }
