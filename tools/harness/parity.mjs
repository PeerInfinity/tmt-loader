...
export async function parity(id, { ticks, diff, leg = 'idle', base, browser, mutant = false }) {
  ...
  return {
    ...
    ticks: node.ticks, gameSeconds: node.gameSeconds, diff, leg,
    node: { ticks: node.ticks, gameSeconds: node.gameSeconds, hash: node.hash, jsonHash: hash16(nodeJson) },
    page: { ticks: page.ticks, gameSeconds: page.gameSeconds, hash: page.hash, ms: page.ms, blocked: page.blocked, failed: page.failed, pageErrors: page.pageErrors },
    divergence: div,
  };
}
...
async function main() {
  ...
  let r;
  try { r = await parity(id, { ticks: Number(a.ticks ?? 1000), diff: Number(a.diff ?? 0.05), leg: a.leg || 'idle', mutant: !!a.mutant, base: a.base || server.url, browser }); }
