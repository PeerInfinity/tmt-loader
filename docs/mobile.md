...
**Over the roster, RE-MEASURED at `468da0513`** (CI run 35657732962, 171/171, 0 RED, `ptr` at `all/M26` = **76,931** ticks — F1's rebuilt chain): **276 candidate Decimals, 17 DETECTED rows on 11 cards across 9 games**, 8 of them sharing a value
...
⚠ **THE SIXTH MOVE REBUILT THE WHOLE CHAIN, and the figure came back to its M24 value.** F1 regenerated `snapshots/ptr/all/` as ONE chain from a fresh game under the final table (M26 is now **76,931** ticks, and the pre-F1 set is kept byte-for-byte in `snapshots/ptr/pre-f1/`, every old pin reproducing). DETECTED is **17 rows on 11 cards** and progress **250 on 244** — the M24 readings, not the M25 ones. ⛔ So the sequence over six moves is 16, 15, 15, 17, 15, 17: it is not monotonic, it is not a ratchet, and it RETURNS to earlier values. "The number matches what I remember" is not evidence; only re-deriving it at the new head is.
...
*(U7's tally, before U13 — the current one is in the U13 section below.)* **Over the roster, at `468da0513`** (`ptr` at `all/M26`): **250 progress rows on 244 cards** — 226 upgrades, 23 buyables, **1 challenge**
