...
| `navbar` | `?navbar=1`, **or** `?mobile=1`, which implies it (docs/mobile.md): the loader linked `loader/navbar.css` and ran `loader/navbar.js`, and `<html>` carries `tmt-navbar` once the bar is installed. The bottom nav bar ALONE — wanted on a desktop, where the layout is not. Explicit like `mobile`, with no viewport or pointer sniffing, and the same promise about the game's state |
