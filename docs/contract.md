...
| `layerListUI` | *(navbar)* `{panel, open(), close(), toggle(), isOpen(), refresh(), groups(), chipsOf(layer), cards()}` — the LAYER LIST the bar's first button opens (docs/mobile.md): the overlay element, its open/close, and what it is showing. Absent without the flag, and the bar hides its Layers button until it exists, so `loader/navbar.js` stands alone without `loader/layerlist.js`. PURE UI like the bar — it never writes `player`, registers no timer and adds no clickable id; its reset button and chips call the engine's own `doReset` / `buyUpgrade` / `buyBuyable` / `startChallenge`, which is the same single click the game's own buttons make |
