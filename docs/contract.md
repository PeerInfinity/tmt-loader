...
| `mobile` | `?mobile=1` (docs/mobile.md): the loader linked `loader/mobile.css` and ran `loader/mobile.js`, and `<html>` carries `tmt-mobile` (+ `tmt-mobile-nav` once tier 2 installed). A LAYOUT opt-in only — it touches no `player` key, adds no layer and registers no timer, so a run with it lands on the same tick and hash as one without |
| `mobileUI` | *(mobile)* `{nav, refresh(), entries, seen}` — the nav bar element and what it has discovered; absent without the flag |
