...
function updateLayers() {
  ...
  for (row in OTHER_LAYERS) {
    ...
    for (layer in OTHER_LAYERS[row]) OTHER_LAYERS[row][layer] = OTHER_LAYERS[row][layer].layer;
  }
  for (row in TREE_LAYERS) {
    TREE_LAYERS[row].sort((a, b) => (a.position > b.position ? 1 : -1));
    for (layer in TREE_LAYERS[row]) TREE_LAYERS[row][layer] = TREE_LAYERS[row][layer].layer;
  }
  let treeLayers2 = [];
...
function setupLayer(layer) {
  ...
  if (layers[layer].upgrades) {
    ...
    for (thing in layers[layer].upgrades) {
      if (isPlainObject(layers[layer].upgrades[thing])) {
        ...
        if (layers[layer].upgrades[thing].unlocked === undefined) layers[layer].upgrades[thing].unlocked = true;
      }
    }
  }
  if (layers[layer].milestones) {
    for (thing in layers[layer].milestones) {
      if (isPlainObject(layers[layer].milestones[thing])) {
        ...
        if (layers[layer].milestones[thing].unlocked === undefined) layers[layer].milestones[thing].unlocked = true;
      }
    }
  }
  if (layers[layer].achievements) {
    ...
    for (thing in layers[layer].achievements) {
      if (isPlainObject(layers[layer].achievements[thing])) {
        ...
        if (layers[layer].achievements[thing].unlocked === undefined) layers[layer].achievements[thing].unlocked = true;
      }
    }
  }
  if (layers[layer].challenges) {
    ...
    for (thing in layers[layer].challenges) {
      if (isPlainObject(layers[layer].challenges[thing])) {
        ...
        if (layers[layer].challenges[thing].unlocked === undefined) layers[layer].challenges[thing].unlocked = true;
        if (layers[layer].challenges[thing].completionLimit === undefined) layers[layer].challenges[thing].completionLimit = 1;
  ...
  if (layers[layer].buyables) {
    ...
    for (thing in layers[layer].buyables) {
      if (isPlainObject(layers[layer].buyables[thing])) {
        ...
        if (layers[layer].buyables[thing].unlocked === undefined) layers[layer].buyables[thing].unlocked = true;
  ...
  if (layers[layer].clickables) {
    ...
    for (thing in layers[layer].clickables) {
      if (isPlainObject(layers[layer].clickables[thing])) {
        ...
        if (layers[layer].clickables[thing].unlocked === undefined) layers[layer].clickables[thing].unlocked = true;
      }
    }
  }
  ...
  if (layers[layer].bars) {
    ...
    for (thing in layers[layer].bars) {
      ...
      if (layers[layer].bars[thing].unlocked === undefined) layers[layer].bars[thing].unlocked = true;
    }
  }
  ...
  if (layers[layer].infoboxes) {
    for (thing in layers[layer].infoboxes) {
      ...
      if (layers[layer].infoboxes[thing].unlocked === undefined) layers[layer].infoboxes[thing].unlocked = true;
    }
  }

  if (layers[layer].grid) {
    layers[layer].grid.layer = layer;
    if (layers[layer].grid.getUnlocked === undefined) layers[layer].grid.getUnlocked = true;
    if (layers[layer].grid.getCanClick === undefined) layers[layer].grid.getCanClick = true;
  }
