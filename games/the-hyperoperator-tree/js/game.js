...
function doReset(layer, force=false) {
	if (tmp[layer].type == "none" || tmp[layer].baseAmount.lt(tmp[layer].requires) || (tmp[layer].baseAmount.lt(tmp[layer].nextAt) && tmp[layer].type === "static") || (tmp[layer].type === "custom" && !tmp[layer].canReset)) return
...
function gameLoop(diff) {
	...
	for (let x = 0; x <= maxRow; x++){
		for (item in TREE_LAYERS[x]) {
			...
			if(layer === "a"){
				layers.a.generateFromDimensions(diff)
			}
			if(layer === "m"){
				layers.m.getTimeSinceReplicant(diff)
				layers.m.gainMultPower(diff)
				layers.m.replicate(diff)
			}
		}
	}
...
var interval = setInterval(function() {
	...
	player.lastResetted += diff
	if(player.lastResetted > 0.1)player.resetting = false
	player.time = now
