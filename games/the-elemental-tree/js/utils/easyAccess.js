...
// ** ALL FUNCTIONS BELOW HAVE BEEN ADDED **
function milestoneEffect(layer, id) {
	return (tmp[layer].milestones[id].effect)
}

function hasSpell(layer, id) {
	return ((player[layer].spells.includes(toNumber(id)) || player[layer].spells.includes(id.toString())) && !tmp[layer].deactivated)
}

function hasMagicLevel(layer, id, lv) {
	if (player[layer].magicLevels.includes(id)) {
		return (player[layer].magicLevels[id].level.gte(lv))
	} else {
		return false
	}
}

function castingSpell(layer, id) {
	if (player[layer].spells.include(id)) {
		return (player[layer].spells[id].isCasting)
	} else {
		return false
	}
}