function exponentialFormat(num, precision, mantissa = true) {
	...
	if (mantissa)
		return parseFloat(m.toStringWithDecimalPlaces(precision)) + "e" + e;
	else return "e" + e;
}
...
function suffixFormat(num, precision) {
	for (let i = SUFFIXES.length - 1; i >= 3; i--) {
		const threshold = Decimal.pow(10, i * 3);
		if (num.gte(threshold)) {
			const divided = num.div(threshold);
			if (divided.gte(1000)) return exponentialFormat(num, precision);
			const formatted = parseFloat(
				divided.toStringWithDecimalPlaces(precision)
			);
			return formatted + SUFFIXES[i];
		}
	}
	return commaFormat(num, precision);
}
...
function format(decimal, precision = 2, small) {
	...
	if (decimal.gte("eeee1000")) {
		...
		return !options.ScientificNotation
			? suffixFormat(decimal, precision)
