...
function loadVue() {
	...
	Vue.component('upgrade', {
		props: ['layer', 'data'],
		template: `
		<button v-if="tmp[layer].upgrades && tmp[layer].upgrades[data]!== undefined && (tmp[layer].upgrades[data].unlocked || hasUpgrade(layer, tmp[layer].upgrades[data].id))" :id='"upgrade-" + layer + "-" + data' v-on:click="buyUpg(layer, data)" v-bind:class="{ [layer]: true, tooltipBox: true, upg: true, bought: hasUpgrade(layer, data), persisting: tmp[layer].upgrades[data].persisting && hasUpgrade(layer, data), locked: (!(canAffordUpgrade(layer, data))&&!hasUpgrade(layer, data)), can: (canAffordUpgrade(layer, data)&&!hasUpgrade(layer, data))}"
	...
	Vue.component('clickable', {
		...
		<button 
			...
			v-bind:class="{ upg: true, tooltipBox: true, can: tmp[layer].clickables[data].canClick, lockedclickable: !tmp[layer].clickables[data].canClick}"
	...
	Vue.component('achievement', {
		props: ['layer', 'data'],
		template: `
		<div v-if="tmp[layer].achievements && tmp[layer].achievements[data]!== undefined && tmp[layer].achievements[data].unlocked" v-bind:class="{ [layer]: true, achievement: true, tooltipBox:true, achievementlocked: !hasAchievement(layer, data), achievementunlocked: hasAchievement(layer, data)}"
