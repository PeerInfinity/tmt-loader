var systemComponents = {
	...
	'overlay-head': {
		...
		<br>
		<span v-if="canGenPoints()"  class="overlayThing">({{tmp.other.oompsMag != 0 ? format(tmp.other.oomps) + " OOM" + (tmp.other.oompsMag < 0 ? "^OOM" : tmp.other.oompsMag > 1 ? "^" + tmp.other.oompsMag : "") + "s" : formatSmall(getPointGen())}}/sec)</span> (x{{formatSmall(player.timeDilationScale)}} Time Dilation)
    ...
    'info-tab': {
		...
		<br><br><br><br>
		<h2><goop>Silly Stats</goop></h2><small><gray> Don't worry this one's safe to touch</gray></small>
		<br><br>
		Total Prestige Times (Counts Automated Prestiges): {{formatWhole(player.sillyStats.prestigeTimes)}}
		<br>
		Total Dialogues Triggered: {{formatWhole(player.dialoguesTriggered)}}
		</div>
    `
    },
    ...
    'options-tab': {
        ...
        <table>
			...
			<tr>
				...
				<td><button class="opt" onclick="toggleOpt('resourcePanelShow')">Enable Resource Panel</button></td>
            </tr>
        </table>`
    },
